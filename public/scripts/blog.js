...
document.addEventListener("DOMContentLoaded", () => {
  ...
  <p class="text-sm text-idcText mb-4">
    <i class="fas fa-calendar-alt"></i> ${new Date(blog.date).toDateString()}
  </p>
  <img src="${blog.image}" class="rounded-xl shadow-lg mb-6 w-full" alt="${
    blog.title
  }" />
  <div class="prose prose-lg text-idcText">${blog.content}</div>
  <h3 class="text-2xl font-bold text-idcPrimary mt-10 mb-4">
  ...
  <form id="add-comment-form" class="mt-6 bg-idcAccent p-6 rounded-lg shadow-md">
    <h4 class="text-xl font-bold text-idcText mb-4">
